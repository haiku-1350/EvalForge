...
from .grounding import GroundingClaim, GroundingReport, review_grounding_content
...
GROUNDING_SYSTEM_PROMPT = """你负责判断 RAG 最终回答中的每项陈述是否得到检索内容支持。只判断支持关系，不要求回答覆盖检索内容的全部信息，也不使用外部知识补足证据。

把回答拆成可核验陈述，status 只能是：
- supported：检索内容完整支持；context_evidence 必须引用检索原文。
- partially_supported：检索内容仅支持一部分；context_evidence 必须引用检索原文。
- unsupported：检索内容不支持；context_evidence 必须为 null。
- non_factual：礼貌用语、建议咨询或明确表示无信息等非事实陈述；context_evidence 必须为 null。

answer_evidence 必须逐字引用最终回答。检索内容为空时，任何资料性事实都必须是 unsupported；诚实说明没有信息可以是 non_factual。

评分：5=全部事实陈述有完整支持，或回答只诚实说明没有信息；4=仅有轻微部分支持，没有无支持事实；3=多数事实有支持但存在明显缺口；2=仅少数事实有支持；1=几乎没有支持；0=所有实质事实均无支持。不得因为回答遗漏检索内容而扣 groundedness 分。
  ...
  "claims": [
    {
      "claim": "单个陈述",
      "status": "supported",
      "answer_evidence": "最终回答原文",
      "context_evidence": "检索内容原文",
      "explanation": "支持关系说明"
    }
  ],
...
class GroundednessEvaluationResult:
    ...
    attempts: int = 0
    model: str = MODEL_A
    stage: str = "groundedness_model_a_initial"
    status: str = "accepted_model_a"
    review_history: tuple[EvaluationAttempt, ...] = ()
...
class DeepSeekEvaluator:
    ...
    def evaluate_groundedness(
        ...
        display_context = context_text or "（无检索内容）"
        base_prompt = (
            "请评估最终回答是否得到检索内容支持。\n"
            f"问题编号：{question_id}\n"
            f"原问题：{question}\n"
            f"检索内容：{display_context}\n"
            f"最终回答：{answer}"
        )
        history: list[EvaluationAttempt] = []
        first_report, first_attempt = self._call_grounding_and_review(
            model=self.model_a,
            stage="groundedness_model_a_initial",
            user_prompt=base_prompt,
            api_url=API_URL,
            api_key=self._api_key,
            question_id=question_id,
        ...
        history.append(first_attempt)
        if first_report.acceptable:
            return self._build_groundedness_result(
                first_report,
                model=self.model_a,
                stage="groundedness_model_a_initial",
                status="accepted_model_a",
        ...
        correction_reasons = list(first_report.issues)
        if first_report.uncertain:
            correction_reasons.extend(
                f"judge_uncertain: {reason}"
                for reason in first_report.uncertainty_reasons
            )
        correction_prompt = (
            base_prompt
            + "\n\n上一次输出未通过 Python 硬校验或存在不确定性："
        ...
        corrected_report, corrected_attempt = self._call_grounding_and_review(
            model=self.model_a,
            stage="groundedness_model_a_correction",
            user_prompt=correction_prompt,
            api_url=API_URL,
            api_key=self._api_key,
            question_id=question_id,
        ...
        self._ensure_model_b_configured()
        blind_report, blind_attempt = self._call_grounding_and_review(
            model=self.model_b,
            stage="groundedness_model_b_blind",
            user_prompt=base_prompt,
            api_url=self._model_b_api_url,
            api_key=self._model_b_api_key,
            question_id=question_id,
        ...
        if (
            blind_report.acceptable
            and corrected_report.passed
            and blind_report.score is not None
            and corrected_report.score is not None
            and abs(blind_report.score - corrected_report.score) >= 2
        ):
            review_reasons.append(
                "judge_disagreement: groundedness 有效评分相差至少 2 分"
            )
        ...
        return self._build_groundedness_result(
            ...
            model=self.model_b,
            stage="groundedness_model_b_blind",
            status="needs_human_review" if review_reasons else "accepted_model_b",
            history=history,
            needs_review=bool(review_reasons),
            extra_review_issues=tuple(review_reasons),
        )

    def _call_grounding_and_review(
        self,
        *,
        model: str,
        stage: str,
        ...
        retrieved_context: str,
        answer: str,
    ) -> tuple[GroundingReport, EvaluationAttempt]:
        content, duration_ms, usage = self._request_content(
            api_url=api_url,
            api_key=api_key,
            model=model,
        ...
        report = review_grounding_content(
            content,
            question_id,
            retrieved_context,
            answer,
    ...
    def _build_groundedness_result(
        ...
        *,
        ...
        return GroundednessEvaluationResult(
            score=report.score,
            reason=report.reason or "groundedness 输出未通过校验，需人工处理。",
